...
paging_init (void)
{
  uint32_t *pd, *pt;  /* pd = page directory, pt = page table */
  size_t page;
  extern char _start, _end_kernel_text;

  pd = init_page_dir = palloc_get_page (PAL_ASSERT | PAL_ZERO); /* virtual address */
  pt = NULL;
  for (page = 0; page < init_ram_pages; page++)
    {
      uintptr_t paddr = page * PGSIZE;
      char *vaddr = ptov (paddr);
      size_t pde_idx = pd_no (vaddr); /* page directory index */
      size_t pte_idx = pt_no (vaddr); /* page table index */
